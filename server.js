const express = require('express');
   ...
   const GOOGLE_DRIVE_FOLDER_ID = '1l4e6cq0LaFS2IFkJlWKLFJ_CVIEqPqTK'; // Replace with your actual folder ID
   ...
   // Download the Excel file from Google Drive on server start - Temporarily disabled
   ...
   (async () => {
     // await initializeFromGoogleDrive(); // Skip Google Drive download on startup
     const workbook = await initializeExcel(); // Initialize a new Excel file locally
     await workbook.xlsx.writeFile(LOCAL_EXCEL_FILE); // Create the file if it doesn't exist
     console.log('Initialized new local Excel file:', LOCAL_EXCEL_FILE);

     // startGoogleDriveSync(); // Temporarily disabled to avoid file conflicts
     app.listen(PORT, () => {
